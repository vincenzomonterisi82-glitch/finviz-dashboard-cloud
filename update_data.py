...
for row in payload.get('results',[]):
    row['last_earnings_date']='N/D'; row['next_earnings_date']='N/D'
    try:
        dates=yf.Ticker(row['ticker']).get_earnings_dates(limit=12)
        if dates is not None and not dates.empty:
            parsed=[]
            for value in dates.index:
                stamp=value.to_pydatetime() if hasattr(value,'to_pydatetime') else value
                parsed.append(stamp.date() if hasattr(stamp,'date') else stamp)
